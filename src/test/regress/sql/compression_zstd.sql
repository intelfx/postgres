...
-- has no effect, the compression method is taken from the table copied.
...
\set VERBOSITY terse
SET compression_zstd_level = 100;
\set VERBOSITY default

-- every level has to round-trip to the same value, inline and external
CREATE TABLE cmlevel(level int, f1 text COMPRESSION zstd);
SET compression_zstd_level = 1;
INSERT INTO cmlevel VALUES (1, repeat('123456789', 1004)),
                           (1, repeat('123456789', 4004));
SET compression_zstd_level = 19;
INSERT INTO cmlevel VALUES (19, repeat('123456789', 1004)),
                           (19, repeat('123456789', 4004));
SET compression_zstd_level = 0;
INSERT INTO cmlevel VALUES (0, repeat('123456789', 1004)),
                           (0, repeat('123456789', 4004));
RESET compression_zstd_level;
SELECT length(f1), count(*), count(DISTINCT f1) FROM cmlevel
  GROUP BY length(f1) ORDER BY 1;
SELECT DISTINCT pg_column_compression(f1) FROM cmlevel;
SELECT DISTINCT SUBSTR(f1, 2000, 20) FROM cmlevel;
